...
func sample(
  ...
  let response = try await client.getLineageEvent(
    request: GetLineageEventRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/processes/\(processId)/runs/\(runId)/lineageEvents/\(lineageEventId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        processId: "[placeholder]", runId: "[placeholder]", lineageEventId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
